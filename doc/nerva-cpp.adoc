...
The `mlp` tool uses this timer `nerva_timer` to keep track of the time spent on feedforward, backpropagate, optimize calls during training, and possibly of some other computations. Each computation is identified with a unique name. If the option `--timer=brief` is set, the accumulated times of all computations will be displayed:
[listing]
----
--- timing results ---
