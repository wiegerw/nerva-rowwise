...
This section gives an overview of the command line interface of the `{mlptool}` tool. It has several options that take a list of items as input. The items of such a list are always separated by semicolons.
Some values like take parameters. We use named parameters, e.g. `AllReLU(alpha=0.3)`. In case that there is only one parameter, the name may be omitted: `AllReLU(0.3)`. If the parameters have default values, they may be omitted. For example, `TReLU` corresponds to `TReLU(al=0,tl=0,ar=0,tr=1)`.
// end::mlptool-options[]
...
for (unsigned int epoch = 0; epoch < options.epochs; ++epoch)
{
  ...
  eta = learning_rate->operator()(epoch);
...
- When using dropout layers, periodically the dropout masks should be renewed. This is typically done after each epoch or after a given number of batches.
- When doing dynamic sparse training, periodically a number of small weights is pruned, and new weights are added back (regrown). This is also typically done after each epoch or after a given number of batches.
...
----
Four actions take place at the start of every epoch.
...
- Small weights in the subnormal range are clipped to zero if the corresponding option is set.
...
NOTE: The sparse CSR matrix functions in the MKL library take an argument of the opaque type `sparse_matrix_t`. It stores unspecified properties of a sparse matrix. This parameter is poorly documented, and it is unknown when this parameter should be recalculated. For safety reasons, the {library} library recalculates this parameter after every change to a sparse matrix, which may cause some inefficiencies. See also the function `sparse_matrix_csr::construct_csr`.
...
To support mini-batches, the {library} library defines all equations that play a role in the execution of a neural network in matrix form, including the backpropagation equations, see the specification document https://wiegerw.github.io/nerva-rowwise/pdf/nerva-libraries-implementation.pdf[Nerva library specifications]. For the latter, many neural network frameworks rely on https://en.wikipedia.org/wiki/Automatic_differentiation[Automatic differentiation], see also cite:[DBLP_BaydinPRS17]. We use explicit backpropagation equations to implement truly sparse layers and to provide an instructive resource for those studying neural network execution.
...
----
On https://groups.google.com/g/llvm-dev/c/TDGKHFU4hzE/m/k-LEa3NvBQAJ[Google Groups] this problem is discussed. A possible solution is to instruct the compiler to flush subnormal values to zero. But there doesn't seem to be a portable way to achieve this. In the {library} library different solutions have been tried. One of them is to periodically flush weights in the subnormal range to zero using the `--clip` command line option of the `mlp` tool.
In cite:[DBLP_HighamM22] the problem of subnormal numbers is discussed.

== Sparse neural networks
Sparse neural network layers are often simulated using binary masks, see cite:[DBLP_abs-2102-01732]. This is caused by the lack of support for sparse tensors in popular neural network frameworks. Note that PyTorch is currently developing https://pytorch.org/docs/stable/sparse.html[sparse tensors]. The {library} library supports truly sparse layers, meaning that the weight matrices of sparse layers are stored in a sparse matrix format. Another example of truly sparse layers is given by cite:[DBLP_NikdanPIKA23].
...
In SET, the sparsity is not divided evenly over the sparse layers. Instead, small layers are assigned a higher density than larger ones.
 In cite:[DBLP_LiuMMPP21] formula (3), https://en.wikipedia.org/wiki/Erd%C5%91s%E2%80%93R%C3%A9nyi_model[Erdős–Rényi graph topology]
...
In {library} this is implemented in the function `compute_sparse_layer_densities`, see `link:https://github.com/wiegerw/nerva-rowwise/blob/main/include/nerva/neural_networks/layer_algorithms.h[layer_algorithms.h]`. The original Python implementation can be found
...
In the {library} library, the classes `prune_function` and `grow_function` are used to represent generic pruning and growing strategies:
[.small-code]
[source,cpp]
----
