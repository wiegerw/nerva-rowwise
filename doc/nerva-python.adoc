...
== Introduction
This document describes the implementation of the `nerva` Python module, that contains bindings to the {library} C++ library. Note that the matrix type used internally in the `nerva` module is `torch.Tensor`.
...
{library} library, and some explanations about the code.

=== Number type
The {library} library uses 32-bit floats as its number type. The C++ library also supports 64-bit floats.
