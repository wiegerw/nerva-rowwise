...
link:nerva-cpp.adoc#cmake-build[CMake install] section in the C++ documentation. The `cmake` command causes the three libraries to be downloaded in the `_deps` subdirectory of the CMake build directory. After that only the environment variable `CMAKE_DEPS_DIR` needs to be set.
