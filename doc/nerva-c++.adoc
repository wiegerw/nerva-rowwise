...
For more background information, see the section called `Matrix operations` in the specification document https://wiegerw.github.io/nerva-rowwise/pdf/nerva-libraries-implementation.pdf[Nerva library specifications]. Using this table leads to concise and uniform code. For example, the backpropagation implementation of a softmax layer looks like this:
[.small-code]
[source,cpp]
----
...
=== Eigen library
The {library} library uses the https://eigen.tuxfamily.org/[Eigen library] for representing matrices. The matrix operations in table <<table_matrix_operations>>
have been implemented using Eigen, see the file https://github.com/wiegerw/nerva-rowwise/blob/main/include/nerva/neural_networks/matrix_operations.h[matrix_operations.h].

== Performance
The performance of training a neural network largely depends on the calculation of matrix products during the backpropagation step of linear layers. In order to do this efficiently, the https://en.wikipedia.org/wiki/Math_Kernel_Library[Intel Math Kernel library (MKL)] is used. Note that the MKL library is included in the https://www.intel.com/content/www/us/en/developer/tools/oneapi/base-toolkit-download.html[Intel oneAPI base toolkit]. The Eigen library supports MKL by means of the compiler flag `EIGEN_USE_MKL_ALL`, see also https://eigen.tuxfamily.org/dox/TopicUsingIntelMKL.html[TopicUsingIntelMKL.html].

=== Intel MKL library
The Intel MKL library has a number of highly efficient, but extremely low-level implementation of matrix operations. See https://www.intel.com/content/www/us/en/docs/onemkl/developer-reference-c/2024-2/blas-and-sparse-blas-routines.html[blas-and-sparse-blas-routines.html] for an overview. The {library} contains some matrix classes that hide those low-level details.
...
NOTE: The sparse CSR matrix functions in the MKL library use an argument of the opaque type `sparse_matrix_t`. It is used to store unknown properties of a sparse matrix. This parameter is poorly documented, and it is unknown when this parameter should be recalculated. For safety reasons, the {library} library recalculates this parameter after every change to a sparse matrix. See also the function `sparse_matrix_csr::construct_csr`.

=== Nerva computation mode
In general, the performance of Eigen is very good. But occasionally, the generated code for a matrix expression can be quite poor. Especially in case of backpropagation calculations this can have a huge impact on the performance. The {library} library uses a global setting `NervaComputation` to experiment with customized implementations. For example, the function `softmax_layer::feedforward` contains this:
[.small-code]
[source,cpp]
----
