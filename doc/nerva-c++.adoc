...
output `Y`.
. *backpropagation:* Given output `Y` corresponding to input `X` and targets `T`, compute the gradient of the loss `DY`. Then from `Y` and `DY`, compute the gradient `DΘ` of the parameters `Θ`.
...
In the command line tool `mlp` the user can select specific implementations of these prune and grow functions. They are called at the start of each epoch of training via an attribute `regrow_function` that applies pruning and growing to an MLP. See also the <<on_start_epoch>> event.
