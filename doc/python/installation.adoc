...
The {library} Python bindings provide access to the high-performance {cpp} backend of the Nerva neural network library. These bindings are built using `pybind11` and compiled with {cpp}17 or higher. Installation is supported via `pip`, but requires several system-level dependencies to be available.
...
Refer to the link:nerva-rowwise.html#cmake-build[CMake install] section in the C++ documentation for details.
